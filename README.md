...
- **多渠道管理**:添加/编辑/删除渠道(`base_url` + `api key`),支持自定义鉴权头(如 `x-api-key`),渠道可启用/停用、设置全局优先级,支持拖拽排序批量调整;支持**渠道级超时 / 冷静期**(毫秒,0 回退全局配置;渠道级超时仅对流式 TTFB 生效,非流式统一由全局 `non_stream_timeout` 约束)。
...
- **智能路由(核心)**:请求按优先级分发;失败先重试同一渠道一次,仍失败则按优先级降级到下一渠道;**连续失败**(默认 3 次,请求成功会清零失败计数)触发**渠道冷静期**(默认 10 分钟),期间该渠道不参与路由,到期自动恢复,也可手动解除;**全部渠道失败时返回详细错误**,逐一列出各渠道名与失败原因(如 `channelA(timeout); channelB(upstream HTTP 503)`),前端日志同步展示失败渠道。
- **模型可用性测试**:对模型的每个关联渠道逐一发送最小流式请求,测出**连通性 / 首字延迟(TTFT)/ 回复速度(token/s)**,方便挑选最优渠道。
- **OpenAI 兼容 API**:对外提供 `/v1/*` 接口(chat/completions、models 等),支持非流式与 SSE 流式,流式末尾 chunk 的 `usage` 会被截获计入统计;接口为**透传性质**,具体能力取决于上游渠道。`/v1/*` 已开启 **CORS**(固定返回 `Access-Control-Allow-Origin: *`),浏览器端应用(本地网页、Web 工具)可直接跨域调用;流式转发时会**过滤上游返回的 CORS 头**,避免与网关设置的头合并成多值被浏览器拦截;管理接口 `/api/admin/*` 不开 CORS 以保安全。
- **三类超时独立控制**:`upstream_timeout`(流式首次响应/TTFB)、`non_stream_timeout`(非流式完整超时,默认 5 分钟)、`stream_max_duration`(流式最长持续时间,默认 6 分钟),互不干扰;流式收到首包后不会被 TTFB 超时打断(详见[配置说明](#配置说明configjson))。
...
- **请求日志(管理后台概览)**:按 **渠道 / 模型 / 状态 / 密钥名称 / 关键词(IP、Request ID)** 多维筛选,分页(默认每页 10 条);tokens 列展示 **P(输入)/ C(输出)/ T(总计)** 三档(悬停可看完整说明,蓝色数字为命中缓存的输入 tokens);**Token 速度列**——输出 tokens ÷ 输出耗时(从首次响应到请求结束),直观反映生成速率;支持**一键清空全部请求日志**(统计聚合保留,不可恢复);失败日志展示**失败渠道名**,错误信息自动**中文化翻译**(超时 / 连接被拒 / DNS 解析失败 / TLS 握手失败等)。
- **API key 管理**:签发多把可命名、可撤销的网关密钥(SHA-256 哈希存储,**明文完整保存在本地数据库**,管理界面可随时查看/复制);日志中记录每次请求所用的密钥名称,便于按密钥筛选排查。
- **零依赖部署**:Go 单二进制(前端 embed)+ SQLite(纯 Go 驱动,免 CGO),Windows / Linux 交叉编译产物开箱即用;Windows 版 `gateway.exe` 自带**应用图标与版本信息**(构建时自动生成,无需手动处理)。
- **管理后台**:内置 Vue3 管理界面(概览看板/日志/渠道/模型/价格/API key),界面文案中文化,仅限本机访问,无需登录。
...
- `gateway.exe` — Windows amd64(带图标与版本信息)
...
脚本会依次:构建前端(`web/npm run build`)→ 编译 Go 单二进制;构建 Windows 目标时会自动生成 `gateway.exe` 的图标与版本信息资源(`cmd/gateway/rsrc_windows_amd64.syso`,由 `go-winres` 生成,缺失时自动重建)。
...
| cooldown_threshold | 3 | **连续**失败多少次触发冷静;请求成功会清零失败计数(偶发失败不累计,中间有成功则重新计数) |
...
> **超时语义**(三类独立,互不干扰):
> - `upstream_timeout` = **流式首次响应超时(TTFB)**:仅对流式请求生效,指请求发起后到收到第一个响应的时间;**流式收到首包后不再受此限制**,输出不会被它打断(旧版本把该值误用于整个流式过程,导致未超时的长流式请求被误判超时,已修复);
...
> **渠道级超时/冷静期**:渠道的创建/编辑弹窗支持按渠道单独配置「超时(ms)」与「冷静期(ms)」,单位**毫秒**,填 `0`(或留空)表示沿用上表全局配置。**渠道级超时仅对流式请求的首次响应(TTFB)生效**(默认 60000ms);**非流式请求一律由全局 `non_stream_timeout` 约束,不受渠道级超时影响**(v0.6.0 起,避免渠道级超时过小误杀正常的长响应)。
...
7. **查看日志**:概览 → 请求日志,支持按渠道 / 模型 / 状态 / 密钥名称 / 关键词筛选;tokens 列(输入/输出/总计)与 Token 速度列帮助判断用量与生成速率;点击行可展开查看请求/响应详情;失败请求会展示失败渠道名与中文化的错误原因。
...
```
cmd/gateway/            入口:启动 HTTP 服务、初始化 DB、挂载前端(含 Windows 资源 syso)
...
web/public/             前端静态资源(favicon、app-icon,构建时复制到 dist)
assets/icon/            应用图标源文件(app_icon_512.png、favicon.ico)与生成说明
scripts/gen_icon.py     图标生成脚本(渐变分流箭头)
```
...
- 前端产物通过 `//go:embed` 打进二进制;Windows 图标/版本信息由 `go-winres` 生成 syso 自动链接
...
→ 取第一个渠道发送:
    ├─ 成功 → 清零该渠道失败计数 → 记录日志/用量 → 返回
    ├─ 失败(连接错误/超时/5xx/429):
    │    ├─ 第 1 次:重试同一渠道(重放请求体)
    │    ├─ 仍失败 → failure_count+1,达到阈值(默认 3)则进入冷静(cooldown_until = now + cooldown_duration)
    │    └─ 取下一个优先级渠道,重复上述流程
    └─ 业务错误(其余 4xx)→ 直接返回客户端,不计渠道失败
→ 全部渠道失败 → 返回汇总错误:渠道名(原因); 渠道名(原因)…(v0.6.0 起)
...
```
请求 → 判定流式 / 非流式
非流式:context 超时 = 全局 non_stream_timeout(默认 5m),渠道级超时不参与
       约束「首次响应 + 读取完整响应体」全过程
...
```powershell
go test ./...          # 单元测试(路由决策、冷静期、成本计算、代理转发、超时控制、CORS 头过滤)
```
...
POST      /api/admin/logs/clear               清空全部请求日志(统计聚合保留)
...
- 网关对外 API(`/v1/*`)不受此限制,依赖 API key 鉴权;已开启 CORS(仅 `/v1/*`,固定返回 `*`)。若需将网关暴露到公网,请通过反向代理将管理端口与对外端口隔离。
...
- **v0.8.0**:请求日志改为实时状态流转(请求到达即入库「等待中」,流式首包后转「传输中」,结束更新为成功/失败/客户端断开,概览页每 2 秒自动刷新,无需手动点刷新);修复 SSE 流 `data: [DONE]` 后客户端断开被误报为「客户端断开」的问题(DONE 后上游残留尾部数据不再透传)。
- **v0.7.0**:模型同步改为「获取预览 → 勾选 → 同步」两步流程(上游数百模型时不再全量导入,只同步勾选项);请求日志新增渠道尝试链路展示(渠道1(失败)→渠道2(成功),逐渠道原因可见)与「客户端断开」状态(区别于渠道故障)。
- **v0.6.0**:非流式超时统一由全局 `non_stream_timeout` 约束(渠道级超时仅流式 TTFB 生效);全部渠道失败返回带渠道名与原因的汇总错误;流式转发过滤上游 CORS 头、`/v1/*` 固定返回 `*`;前端中文化(状态徽章/筛选/错误翻译);Windows exe 图标与版本信息、网页 favicon;新增 CORS 与超时语义测试。
- **v0.5.0**:冷却阈值默认 3(成功清零失败计数)、日志一键清空、前端交互优化。
- **v0.4.0**:超时三档化(TTFB/非流/流式最长)、日志按密钥筛选与 Token 速度列、README 重写。
- **v0.3.0**:渠道级超时/冷静期、日志上游模型、模型页 UI 优化。
