<template>
  <div>
    <!-- 页头 -->
    <div class="flex md:flex-row justify-between items-center gap-4 mb-8">
      <div class="flex items-center gap-3">
        <div class="w-2 h-8 bg-white"></div>
        <h1 class="text-2xl font-bold tracking-tight uppercase">请求日志</h1>
      </div>
      <div class="flex items-center gap-3">
        ...
        <button @click="load" class="bg-white text-black px-4 py-2 rounded text-xs uppercase hover:bg-neutral-200 font-bold">刷新日志</button>
      </div>
    </div>
    ...
    <div class="grid md:grid-cols-4 gap-4 mb-8">
      <div class="glass-card p-4">
        ...
        <select v-model="filters.channel_id" class="w-full bg-[#1a1a1a] border-[#262626] text-xs px-3 py-2 rounded focus:border-white input-field">
      ...
      <div class="glass-card p-4">
        ...
        <select v-model="filters.model" class="w-full bg-[#1a1a1a] border-[#262626] text-xs px-3 py-2 rounded focus:border-white input-field">
      ...
      <div class="glass-card p-4">
        ...
        <select v-model="filters.status" class="w-full bg-[#1a1a1a] border-[#262626] text-xs px-3 py-2 rounded focus:border-white input-field">
      ...
      <div class="glass-card p-4">
        ...
        <div class="relative">
          ...
          <input v-model="filters.keyword" placeholder="IP, Request ID..."
            class="w-full bg-[#1a1a1a] border-[#262626] text-xs px-3 py-2 rounded pl-9 input-field" @keyup.enter="load" />
        </div>
      </div>
    </div>
...
import { api, fmtNum, fmtCost, fmtTimeShort } from '../api'
...
onMounted(async () => {
  const [c, m] = await Promise.all([api.listChannels(), api.listModels()])
